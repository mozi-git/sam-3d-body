...
class SAM3DBody(BaseModel):
    ...
    def get_ray_condition(self, batch):
        ...
        meshgrid_xy = (
            torch.stack(
                torch.meshgrid(torch.arange(H, device=device), torch.arange(W, device=device), indexing="xy"), dim=2
            )[None, None, :, :, :]
            .repeat(B, N, 1, 1, 1)
            # .cuda()
        )  # B x N x H x W x 2
    ...
    def run_inference(
        ...
        lowarm_joint_idxs = torch.LongTensor([77, 41]).to(joint_rotations.device)  # left, right
        ...
        wrist_twist_joint_idxs = torch.LongTensor([78, 42]).to(joint_rotations.device)  # left, right
        wrist_zero_rot_pose = (
